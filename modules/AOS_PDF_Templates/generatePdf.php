...
if($_REQUEST['module'] == "AOS_Invoices" && $_REQUEST['uid']!="") {

    $result = $bean->db->query("select * from atomi_invoice_payment_details inner join atomi_invoice_payment_details_cstm on atomi_invoice_payment_details.id=atomi_invoice_payment_details_cstm.id_c where atomi_invoice_payment_details_cstm.aos_invoices_id_c = '".$_REQUEST['uid']."'");

    $numRows = $result->num_rows;

    if($numRows > 0) {

        $paymentDetails.='<tr><td colspan="4"></td><td style="border-style: solid;background-color: #b0c4de;border-width: 0.5px; padding: 2px 6px;font-weight: bold;text-align:right;font-size: 8px;">Payment Type</td><td style="border-style: solid; background-color: #b0c4de;border-width: 0.5px; padding: 2px 6px;font-size: 8px;">Transaction Number</td><td style="border-style: solid;background-color: #b0c4de;border-width: 0.5px; padding: 2px 6px;font-size: 8px;">Payment Date</td><td style="border-style: solid; background-color: #b0c4de; border-width: 0.5px; padding: 2px 6px;font-size: 8px;">Amount</td></tr>';
        ...
        while($rowPay = $bean->db->fetchByAssoc($result)) {

            $paymenttype_c = $rowPay['paymenttype_c'];
            $paymentamount_c = $rowPay['paymentamount_c'];

            $paymentdate_c_data = explode("-", $rowPay['paymentdate_c']);
            $paymentdate_c = $paymentdate_c_data[2]."/".$paymentdate_c_data[1]."/".$paymentdate_c_data[0];
            $transactionnumber_c = $rowPay['transactionnumber_c'];

            $paymentDetails.='<tr><td colspan="4"></td><td style="border-style: solid; border-width: 0.5px; padding: 2px 6px; font-weight: bold;text-align:right;font-size: 8px;">'.$payTypeArray[$paymenttype_c].'</td><td style="border-style: solid; border-width: 0.5px; padding: 2px 6px;font-size: 8px;">'.$transactionnumber_c.'</td><td style="border-style: solid; border-width: 0.5px; padding: 2px 6px;font-size: 8px;">'.$paymentdate_c.'</td><td style="border-style: solid; border-width: 0.5px; padding: 2px 6px;font-size: 8px;">'.number_format($paymentamount_c,2).'</td></tr>';
        }
        ...
        $text = preg_replace("/<a\s(.+?)>(.+?)<\/a>/is", $paymentDetails, $text);

        /*$text = str_replace("<tr><td colspan=\"4\"></td><td colspan=\"3\" style=\"border-style: solid; border-width: 0.5px; padding: 2px 6px; font-weight: bold; text-align: right;\">Payments</td><td style=\"border-style: solid; border-width: 0.5px; padding: 2px 6px;\">Amount</td></tr>", $paymentDetails, $text);*/
