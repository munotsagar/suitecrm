    ...
    if($view == 'EditView'){
        ...
        if($focus->id != '') {
            ...
            while ($row = $focus->db->fetchByAssoc($result)) {
                ...
                //print_r($line_item->product_id);

                if($line_item->product_id)
                {
                    $sql = "SELECT *  FROM aos_products inner JOIN aos_products_cstm on aos_products.id = aos_products_cstm.id_c where aos_products.id = '".$line_item->product_id."' order by aos_products.id";
                    //echo $sql;
                    $resultProd = $GLOBALS["db"]->query($sql);
                    while ( $product = $GLOBALS["db"]->fetchByAssoc($resultProd) ) {
                       //print_r(currency_format_number($product[tier_5_price_c]));exit;
                        $line_item->product_hidden_tier_5_price_c = $product[tier_5_price_c];
                        $line_item->product_hidden_tier_4_price_c = $product[tier_4_price_c];
                        $line_item->product_hidden_tier_3_price_c = $product[tier_3_price_c];
                        $line_item->product_hidden_tier_2_price_c = $product[tier_2_price_c];
                        $line_item->product_hidden_tier_1_price_c = $product[tier_1_price_c];
                        $line_item->product_hidden_tier_0_price_c = $product[tier_0_price_c];
                    }
        ...
        while ($row = $focus->db->fetchByAssoc($result)) {
            ...
            if($enable_groups && ($group_id != $row['group_id'] || $i == 0)){
                ...
                $groupEnd .= "<tr>";
                $groupEnd .= "<td class='tabDetailViewDL' colspan='8' style='text-align: right;padding:2px;' 
scope='row'>".$mod_strings['LBL_TOTAL_AMT'].":&nbsp;&nbsp;</td>";
                ...
                $groupEnd .= "<tr>";
                $groupEnd .= "<td class='tabDetailViewDL' colspan='8' style='text-align: right;padding:2px;' 
scope='row'>".$mod_strings['LBL_DISCOUNT_AMOUNT'].":&nbsp;&nbsp;</td>";
                ...
                $groupEnd .= "<tr>";
                $groupEnd .= "<td class='tabDetailViewDL' colspan='8' style='text-align: right;padding:2px;' 
scope='row'>".$mod_strings['LBL_SUBTOTAL_AMOUNT'].":&nbsp;&nbsp;</td>";
                ...
                $groupEnd .= "<tr>";
                $groupEnd .= "<td class='tabDetailViewDL' colspan='8' style='text-align: right;padding:2px;' 
scope='row'>".$mod_strings['LBL_TAX_AMOUNT'].":&nbsp;&nbsp;</td>";
                ...
                $groupEnd .= "<tr>";
                $groupEnd .= "<td class='tabDetailViewDL' colspan='8' style='text-align: right;padding:2px;' 
scope='row'>".$mod_strings['LBL_GRAND_TOTAL'].":&nbsp;&nbsp;</td>";
            ...
            if($line_item->product_id != '0' && $line_item->product_id != null){
                if($productCount == 0)
                {
                    ...
                    $product .= "<td width='12%' class='tabDetailViewDL' style='text-align: left;padding:2px;' scope='row'>".$mod_strings['LBL_PART_NUMBER']."</td>";
                    $product .= "<td width='12%' class='tabDetailViewDL' style='text-align: right;padding:2px;' scope='row'>".$mod_strings['LBL_SELLING_COST']."</td>";
                    $product .= "<td width='12%' class='tabDetailViewDL' style='text-align: right;padding:2px;' scope='row'>".$mod_strings['LBL_PRODUCT_DISCOUNT']."</td>";
                ...
                $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".$line_item->part_number."</td>";

                $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".currency_format_number($line_item->product_tier_5_price_c,$params)."</td>";

                $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".get_discount_string($line_item->product_discount_amount, $line_item->product_discount_amount, $params, $locale, $sep)."</td>";
                ...
                if($locale->getPrecision()){
                    $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".rtrim(rtrim(format_number($line_item->product_tax_percentage_c), '0'),$sep[1])."%</td>";
                } else {
                    $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".format_number($line_item->product_tax_percentage_c)."%</td>";
                }
                $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".currency_format_number($line_item->product_tax_amount_c,$params )."</td>";
